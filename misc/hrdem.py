...
class HRDEMses(Qproj):
    ...
    def retrieve_qgs(self, #get a local copy using QGS objects
        ...
        if ofp is None:
            ofp = os.path.join(self.temp_dir, self.longname + '_hrdem_raw.tif')
...
def get_clip(
    ...
    with HRDEMses(name=name, compression='none', overwrite=True, aoi_set_proj_crs=True, aoi_fp=aoi_fp) as wrkr:
    
        ofp = wrkr.get_hrdem(resolution=resolution, aoi_fp=aoi_fp)
