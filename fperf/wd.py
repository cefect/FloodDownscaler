...
from hp.rio import (
    RioSession, RioWrkr, assert_rlay_simple, _get_meta, assert_spatial_equal, is_raster_file,
...
class ValidatePoints(RioWrkr, GeoPandasWrkr):
    ...
    def __init__(self,
        ...
        if not true_wd_fp is None:
            rlay_ar_apply(true_wd_fp, assert_wsh_ar)
            
            self.true_wd_fp=true_wd_fp
            
        if not pred_wd_fp is None:
            rlay_ar_apply(pred_wd_fp, assert_wsh_ar)
    ...
    def _load_stats(self, fp=None):
        ...
        with rio.open(fp, mode='r') as ds:
            rlay_ar_apply(ds, assert_wsh_ar)
            assert_rlay_simple(ds)
            self.stats_d = _get_meta(ds) 
    ...
    # get bounding box from rasters
        ...
        # clean
        assert index_coln in gdf, f'gdf missing \'{index_coln}\'\n    {fp}'
