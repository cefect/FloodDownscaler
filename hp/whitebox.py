...
class Whitebox(object):
    ...
    def bufferRaster(self,
                        rlay_fp,
                        size=11, #pixel length to infil
                        logger=None, out_fp=None,
                        ):
        """
        TODO: check that some zero values exist
        """

        #=======================================================================
        # defaults
        #=======================================================================
        tool_nm = 'BufferRaster'
        if logger is None: logger=self.logger
        log=logger.getChild(tool_nm)
        
        if out_fp is None: 
            out_fp = os.path.join(self.out_dir, os.path.splitext(os.path.basename(rlay_fp))[0]+'_buff.tif')
